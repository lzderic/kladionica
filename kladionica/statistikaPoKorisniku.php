...
<?php
	...
	if($aktivni_korisnik_tip==0||$aktivni_korisnik_tip==1){
		...
		echo '<form method="GET" action="statistikaPoKorisniku.php">
	...
	if(isset($_GET["liga"]) || isset($_GET["od"]) || isset($_GET["do"])){
		...
		$trenutna.="?korisnik=".$_GET["korisnik"]."&od=".$_GET["od"]."&do=".$_GET["do"]."&";
		$trenutna2.="?korisnik=".$_GET["korisnik"]."&od=".$_GET["od"]."&do=".$_GET["do"]."&";
	...
	FROM listic l INNER JOIN korisnik k ON l.korisnik_id = k.korisnik_id 
	INNER JOIN utakmica u ON u.utakmica_id = l.utakmica_id 
	INNER JOIN momcad m1 ON u.momcad_1 = m1.momcad_id 
	INNER JOIN momcad m2 ON u.momcad_2 = m2.momcad_id 
	INNER JOIN liga lg ON lg.liga_id = m1.liga_id AND lg.liga_id = m2.liga_id 
	WHERE l.`status` IN ('D','N')";
		...
		if(isset($_GET["korisnik"])){
			$nazivkorisnik=$_GET["korisnik"];
			$sql.=" AND (k.ime LIKE '%$nazivkorisnik%' or k.prezime LIKE '%$nazivkorisnik%')";
		}
		if($aktivni_korisnik_tip==1){
			$sql.=" AND lg.moderator_id = ".$aktivni_korisnik_id;
		}
		
		$sql.=" GROUP BY k.korisnik_id) AS nova";
	...
	echo "<table>";
	echo "<caption>Statistika po korisniku</caption>";
	echo "<thead><tr>";
	echo "<th>Korisnik</th>";
	...
	echo "<tbody>";
	while(list($korisnik,$dobitni,$nedobitni)=mysqli_fetch_row($rs)){
		echo "<tr>
			<td>$korisnik</td>
			<td>$dobitni</td>
			<td>$nedobitni</td>
